...
class Graph:
    def __init__(self):
        ...
        self.maxTerminals = 0
        self.myInitialNodes = []
        self.myTerminalNodes = []
        self.finalCorrespondingTerminalArray = []
        # self.reqDictionary = []
        # self.respDistionary = []
        self.cmdList = []
        self.cmdRelatedList = []
    ...
    def read_message_file(self, msg_def_file_name):
        ...
        # Bardia's variables
        rootNodesMessageArray = [""]
        terminalNodesMessageArray = [""]
        # maxInitials  = 0
        # maxTerminals = 0
        correspondingTerminalArray = []
        # Bardia's variables end
        ...
        for line in f1:
            ...
            if tokens[0] == '#':
                ...
                if parse_state == SecType.IDLE:
                    ...
                    parse_state = SecType.STARTENDMATCHES
                    correspondingTerminalArray = [[] for i in range(self.maxInitials+1)]
                    log('StartEndMatches section \n', DEBUG)
                elif parse_state == SecType.STARTENDMATCHES:
                    parse_state = SecType.RELATEDCOMMANDS
                    log('RelatedCommands section \n', DEBUG)
                # elif parse_state == SecType.TERMINAL:
                continue
            ...
            if parse_state != SecType.STARTENDMATCHES and parse_state != SecType.RELATEDCOMMANDS:
                ...
                if not self.has_node(symbol_index):
                    ...
                    self.add_node(node)
                    ####################################################################### By Bardia
                    # if command not in self.respDistionary:
                    #     print("\t\t CHECK\t\t")
                    #     self.respDistionary.append(command)
                    #     tempReqs = []
                    #     self.reqDictionary.append(tempReqs)
                    # if msg_type == "req":
                    #     for com in self.respDistionary:
                    #         checkCommand = com.split("Resp")
                    #         if checkCommand[0] in command:
                    #             if command not in self.reqDictionary[self.respDistionary.index(com)]:
                    #                 self.reqDictionary[self.respDistionary.index(com)].append(command)
                    ####################################################################### By Bardia
                    ...
                    if parse_state == SecType.START:
                        ...
                        ####################################################################### By Bardia
                        if(int(symbol_index)>self.maxInitials):
                            for i in range((int(symbol_index)-self.maxInitials)-1):
                                rootNodesMessageArray.append("")
                            rootNodesMessageArray.append(str(symbol_index)+":"+str(origin)+":"+str(destination)+":"+str(command)+":"+str(msg_type))
                            self.maxInitials = int(symbol_index)
                        else:
                            rootNodesMessageArray[int(symbol_index)] = (str(symbol_index)+":"+str(origin)+":"+str(destination)+":"+str(command)+":"+str(msg_type))

                        self.myInitialNodes.append(int(symbol_index))
                        ####################################################################### By Bardia end
                    elif parse_state == SecType.TERMINAL:
                        ...
                        ####################################################################### By Bardia
                        if(int(symbol_index)>self.maxTerminals):
                            for i in range((int(symbol_index)-self.maxTerminals)-1):
                                terminalNodesMessageArray.append("")
                            terminalNodesMessageArray.append(str(symbol_index)+":"+str(origin)+":"+str(destination)+":"+str(command)+":"+str(msg_type))
                            self.maxTerminals = int(symbol_index)
                        else:
                            terminalNodesMessageArray[int(symbol_index)] = (str(symbol_index)+":"+str(origin)+":"+str(destination)+":"+str(command)+":"+str(msg_type))

                        self.myTerminalNodes.append(int(symbol_index))
                        ####################################################################### By Bardia end
            elif parse_state == SecType.STARTENDMATCHES:
                initial = int(tokens[0])
                terminal = int(tokens[1])
                correspondingTerminalArray[initial].append(terminal)
                # initialMessage  = self.get_node(tokens[0]).get_message()
                # terminalMessage = self.get_node(tokens[1]).get_message()
                # if terminalMessage[3] in self.respDistionary:
                #     if initialMessage[3] not in self.reqDictionary[self.respDistionary.index(terminalMessage[3])]:
                #         self.reqDictionary[self.respDistionary.index(terminalMessage[3])].append(initialMessage[3])
                # else:
                #     self.respDistionary.append(terminalMessage[3])
                #     tempReqs = [initialMessage[3]]
                #     self.reqDictionary.append(tempReqs)
            elif parse_state == SecType.RELATEDCOMMANDS:
                self.cmdList.append(tokens[0])
                tempList = []
        ...
        print("-------------------------------------------")
        self.finalCorrespondingTerminalArray = correspondingTerminalArray
        for i in range(len(self.finalCorrespondingTerminalArray)):
            if correspondingTerminalArray[i]:
                print("i =", i, "and terminals =", self.finalCorrespondingTerminalArray[i])
        for n in self.cmdList:
            print("Command: ", n, end=" ")
            print(" And Related:", end=" ")
            for R in self.cmdRelatedList[self.cmdList.index(n)]:
                print(R, end=" ")
            print(" ")
        # for nodes in self.respDistionary:
        #     print("Terminal command: ", nodes, end=" ")
        #     print(" And initials:", end=" ")
        #     for initialNodes in self.reqDictionary[self.respDistionary.index(nodes)]:
        #         print(initialNodes, end=" ")
        #     print(" ")
        print("-------------------------------------------")

        # ################################################################################################################################################################
        # # print("\nTest!!!!!!!")
        # # for i in range(len(rootNodesMessageArray)):
        # #     print("i=", i, "and variable = ", rootNodesMessageArray[i])
        # # print("-----------------------------------------------------------")
        # # for i in range(len(terminalNodesMessageArray)):
        # #     print("i=", i, "and variable = ", terminalNodesMessageArray[i])
        # # print("-----------------------------------------------------------")

        # # for 
        # # print("Max =", maxInitials)
        # # print("Root node array =", rootNodesMessageArray)
        # # print("Terminal node array =", terminalNodesMessageArray)
        # # maxInitials = 0
        # myInitialNodes = []
        # myTerminalNodes = []
        # for root in self.root_nodes:
        #     myInitialNodes.append(int(root))

        # for terminalNode in self.terminal_nodes:
        #   myTerminalNodes.append(int(terminalNode))


        # # print("Max after =", maxInitials)
        # print("Starting nodes = ", myInitialNodes)
        # print("Ending nodes = ",   myTerminalNodes)

        # correspondingTerminalArray = [[] for i in range(maxInitials+1)]
        # for initial in myInitialNodes:
        #     initialToken = rootNodesMessageArray[initial].split(":")
        #     for terminal in myTerminalNodes:
        #         terminalToken = terminalNodesMessageArray[terminal].split(":")
        #         readFlag = False
        #         writeFlag = False
        #         lockedRMWReadFlag = False
        #         lockedRMWWriteFlag = False

        #         if "gem5" in msg_def_file_name:
        #             #### For Gem5 traces
        #             if ("LockedRMWRead" in initialToken[3]) or ("LockedRMWWrite" in initialToken[3]):
        #                 lockedRMWReadFlag = "LockedRMWRead" in initialToken[3] and "LockedRMWRead" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                 lockedRMWWriteFlag = "LockedRMWWrite" in initialToken[3] and "LockedRMWWrite" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #             else:
        #                 if ("LockedRMWRead" in terminalToken[3]) or ("LockedRMWWrite" in terminalToken[3]):
        #                     lockedRMWReadFlag = "LockedRMWRead" in initialToken[3] and "LockedRMWRead" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                     lockedRMWWriteFlag = "LockedRMWWrite" in initialToken[3] and "LockedRMWWrite" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                 else: 
        #                     readFlag = "Read" in initialToken[3] and "Read" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                     writeFlag = "Write" in initialToken[3] and "Write" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #         else:
        #             #### For Synthetic Traces
        #             if ("uprd" in initialToken[3]) or ("upwt" in initialToken[3]):
        #                 lockedRMWReadFlag = "uprd" in initialToken[3] and "uprd" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                 lockedRMWWriteFlag = "upwt" in initialToken[3] and "upwt" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #             else:
        #                 if ("uprd" in terminalToken[3]) or ("upwt" in terminalToken[3]):
        #                     lockedRMWReadFlag = "uprd" in initialToken[3] and "uprd" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                     lockedRMWWriteFlag = "upwt" in initialToken[3] and "upwt" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                 else: 
        #                     readFlag = "rd" in initialToken[3] and "rd" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"
        #                     writeFlag = "wt" in initialToken[3] and "wt" in terminalToken[3] and initialToken[4] == "req" and terminalToken[4] == "resp"

        #         # print("1:", readFlag, "2:", writeFlag, "3:", lockedRMWReadFlag, "4:", lockedRMWWriteFlag)
        #         if "WritebackDirty" in initialToken[3] and "WritebackDirty" in terminalToken[3]:
        #             correspondingTerminalArray[initial].append(terminal)
        #         elif initialToken[1] == terminalToken[2] and initialToken[2] == terminalToken[1] and (readFlag or writeFlag or lockedRMWReadFlag or lockedRMWWriteFlag):
        #             correspondingTerminalArray[initial].append(terminal)
        #     # print(rootNodesMessageArray[initial])
        # print("-------------------------------------------")
        # self.finalCorrespondingTerminalArray = correspondingTerminalArray
        # for i in range(len(self.finalCorrespondingTerminalArray)):
        #     if correspondingTerminalArray[i]:
    ...
    def generate_edges(self):
        for node_src in self.nodes.values():
            ...
            for node_dest in self.nodes.values():
                ...
                if self.causal(node_src, node_dest):
                    ...
                    self.networkxGraph.add_edge(int(node_src.get_index()), int(node_dest.get_index()))
        ...
        #     print(anEdge, end=", ")
        # # print ("\nAll edges: ", self.edges.get_id())
        # myInitialNodes = []
        # myTerminalNodes = []
        # print("\nInitial nodes: ", myInitialNodes)
        # print("Terminal nodes: ", myTerminalNodes)
        # for aNode in self.networkxGraph:
        #     if (self.networkxGraph.in_degree(aNode) == 0):
        #         myInitialNodes.append(aNode)
        #         # print("Initial node = ", aNode)
        #     if (self.networkxGraph.out_degree(aNode) == 0):
        #         myTerminalNodes.append(aNode)
        #         # print("Terminal node = ", aNode)
        # maxN = 0

        # print("Total number of edges in the graph = ", self.networkxGraph.number_of_edges())
        # print("Initial nodes:")
        ...
        # print("Starting nodes = ", myInitialNodes)
        # print("Ending nodes = ",   myTerminalNodes)
        ############################################# For printing out the causality graph ###############################################
        # plt.figure(figsize=(10, 8))
        # pos = nx.circular_layout(self.networkxGraph)
        # # pos = nx.kamada_kawai_layout(self.networkxGraph)
        # # pos = nx.spectral_layout(self.networkxGraph)
        # # pos = nx.spring_layout(self.networkxGraph, seed=1000)
        ...
        # # nx.draw_networkx_nodes(self.networkxGraph, pos, **node_options)
        # # nx.draw_networkx_edges(self.networkxGraph, pos, **edge_options)
        # # nx.draw_networkx_labels(self.networkxGraph, pos, **node_label_options)
        # nx.draw(self.networkxGraph, pos, with_labels=True)
        # plt.show()
        ############################################# For printing out the causality graph #################### end ######################
        # counter = 0
        ...
        #         counter += 1
        #         for path in nx.all_simple_paths(self.networkxGraph, source=start_node, target=end_node):
    ...
    def process_trace(self, raw_trace):
        ...
        buggyTrace = []
        for token in tokens:
            if not (token == '0' or token == '9'):
                buggyTrace.append(token)
            if token == '-2':
            ...
            if(int(token) not in self.all_messages):
                self.all_messages.append(int(token))
            ...
            self.trace_tokens.append(token)
            thisSpecificTrace.append(token)
        ...
        edges = self.get_edges().values()
        # print("Length edge array:", len(edges))
        ...
        for edge in edges:
            ...
            if str(dest_node) not in thisSpecificTrace: continue
    ...
    #     # elif msg_1.get_command() in self.reqDictionary[self.respDistionary.index(msg_2.get_command())]:
    #     elif msg_1.get_command() in self.cmdRelatedList[self.cmdList.index(msg_2.get_command())]:
    #         if msg_1.get_type().lower() == 'req':
    #             return (msg_2.get_type().lower() != 'resp')
    ...
    # Old causal function (Coded by Dr. Zheng I think)
    def causal(self, msg_1, msg_2):
        if msg_1.get_destination() != msg_2.get_source():
            return False
